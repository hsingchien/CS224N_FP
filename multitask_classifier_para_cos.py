...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.predict_paraphrase_af = nn.Linear(BERT_HIDDEN_SIZE,BERT_HIDDEN_SIZE)
        self.predict_similarity_af = nn.Linear(BERT_HIDDEN_SIZE, BERT_HIDDEN_SIZE)
    ...
    def predict_paraphrase(
        ...
        hidden1 = self.predict_paraphrase_af(hidden1)
        hidden2 = self.predict_paraphrase_af(hidden2)
        hidden1 = self.dropout_layer(F.relu(hidden1))
        hidden2 = self.dropout_layer(F.relu(hidden2))
        sim_cos = self.cos(hidden1, hidden2)
        sim_cos = sim_cos.unsqueeze(-1)
        logits = F.sigmoid(sim_cos-0.5) # to center this to -0.5,0.5
        # hidden = self.dropout_layer(hidden)
        # logits = self.predict_paraphrase_af2(hidden)
        return logits
    ...
    def predict_similarity(
        ...
        avg_hidden1 = self.predict_similarity_af(avg_hidden1)
        avg_hidden2 = self.predict_similarity_af(avg_hidden2)
        avg_hidden1 = self.dropout_layer(F.relu(avg_hidden1))
        avg_hidden2 = self.dropout_layer(F.relu(avg_hidden2))
        ...
        # sim_score = self.predict_similarity_logit(sim_score.unsqueeze(-1))
        return sim_score
    ...
    for epoch in range(args.epochs):
        ...
        for iternum in tqdm(
            ...
            if loss_ratio[1] > 0:
                ...
                para_labels = para_labels.float().to(device)
                para_logits = model.predict_paraphrase(
                    para_ids1, para_mask1, para_ids2, para_mask2
                )
                para_loss = F.binary_cross_entropy(para_logits, para_labels.unsqueeze(-1), reduction="mean")
                # para_loss = (
...
def ttrain_multitask(args):
    ...
    with torch.no_grad():
        ...
        with open(f"{args.prediction_out}sst-train.csv", "w+") as f:
            print(f"train sentiment acc :: {train_sentiment_accuracy :.3f}")
            f.write(f"id , Predicted_Sentiment \n")
            for p, s in zip(train_sst_sent_ids, train_sst_y_pred):
                f.write(f"{p} , {s} \n")

        with open(f"{args.prediction_out}para-train.csv", "w+") as f:
            print(f"train paraphrase acc :: {train_paraphrase_accuracy :.3f}")
            f.write(f"id , Predicted_Is_Paraphrase \n")
            for p, s in zip(train_para_sent_ids, train_para_y_pred):
                f.write(f"{p} , {s} \n")

        with open(f"{args.prediction_out}sts-train.csv", "w+") as f:
            print(f"train sts corr :: {train_sts_corr :.3f}")
            f.write(f"id , Predicted_Similiary \n")
            for p, s in zip(train_sts_sent_ids, train_sts_y_pred):
                f.write(f"{p} , {s} \n")

        with open(f"{args.prediction_out}train_perf.txt", "w+") as f:
            f.write(f"train sentiment acc :: {train_sentiment_accuracy :.3f}\n")
            f.write(f"train paraphrase acc :: {train_paraphrase_accuracy :.3f}\n")
            f.write(f"train sts corr :: {train_sts_corr :.3f}")
